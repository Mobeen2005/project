...
def main():
    ...
    def upload_and_process():
        ...
        # Resize input images and concatenate them horizontally
        input_images_concat = np.zeros((50, 50 * numImages, 3), np.float32)
        for i in range(numImages):
            resized_img = cv2.resize(images[i], (50, 50))
            input_images_concat[:, i * 50:(i + 1) * 50, :] = resized_img

        # Combine the output image and the concatenated input images
        combined_output = np.vstack((output, input_images_concat))
        cv2.imwrite("combined_output.jpg", combined_output * 255)
    ...
    def display_output():
        ...
        # Display the combined output image on the canvas
        combined_output_img = cv2.imread("combined_output.jpg")
        combined_output_img = cv2.cvtColor(combined_output_img, cv2.COLOR_BGR2RGB)
        photo = ImageTk.PhotoImage(Image.fromarray(combined_output_img))
